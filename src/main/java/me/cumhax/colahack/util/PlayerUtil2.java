...
import me.cumhax.colahack.util.Util;
...
public class PlayerUtil2 implements Util {
    ...
    public static BlockPos getPlayerPos(double pY) {
        return new BlockPos(Math.floor(PlayerUtil2.mc.player.posX), Math.floor(PlayerUtil2.mc.player.posY + pY), Math.floor(PlayerUtil2.mc.player.posZ));
    }
    ...
    private static JsonElement getResources(final URL url, final String request, final JsonElement element) throws Exception {
        ...
        try {
            ...
            final JsonElement data = PlayerUtil2.PARSER.parse(json);
            return data;
        } finally {
            if (connection != null) {
    ...
    public static boolean isInHole() {
        BlockPos player_block = PlayerUtil2.getPlayerPos();
        return PlayerUtil2.mc.world.getBlockState(player_block.east()).getBlock() != Blocks.AIR && PlayerUtil2.mc.world.getBlockState(player_block.west()).getBlock() != Blocks.AIR && PlayerUtil2.mc.world.getBlockState(player_block.north()).getBlock() != Blocks.AIR && PlayerUtil2.mc.world.getBlockState(player_block.south()).getBlock() != Blocks.AIR;
    }
        ...
        public void run() {
            ...
            if (profile == null) {
                LoggerUtil.sendMessage("Player isn't online. Looking up UUID..");
                final String s = PlayerUtil2.requestIDs("[\"" + this.name + "\"]");
                if (s == null || s.isEmpty()) {
                    LoggerUtil.sendMessage("Couldn't find player ID. Are you connected to the internet? (0)");
                } else {
                    final JsonElement element = new JsonParser().parse(s);
                    if (element.getAsJsonArray().size() == 0) {
                        LoggerUtil.sendMessage("Couldn't find player ID. (1)");
                    } else {
                        try {
                            ...
                            e.printStackTrace();
                            LoggerUtil.sendMessage("Couldn't find player ID. (2)");
                        }
                    }
                }
            }
